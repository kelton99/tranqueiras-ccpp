...
int dl_list_size(DLinked_List *list)
{
        return list->size;
}
...
void dl_list_remove(DLinked_List *list, int value)
{
        if(dl_is_empty(list)) return;
        Node *aux_node = list->head;
...
void dl_list_remove_from(DLinked_List *list, int index)
{
        if(list->size <= index || index < 0){
                puts("Invalid index");
                return;
